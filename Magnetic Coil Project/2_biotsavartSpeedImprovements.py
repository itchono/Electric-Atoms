...
def testOLD():
    ...
    print("Slices generated in {}s".format(time.perf_counter()-t_0))
    ...
    for i in range(10):
        ...
        B1 = calculateFieldOLD(chopped, 1, (5+i, 5+i, 5+i))
...
def testNEW():
    ...
    for i in range(10):
        t_start = time.perf_counter_ns()
        B2 = calculateField(chopped, 1, np.array((5+i, 5+i, 5+i)))
        t_end = time.perf_counter_ns()
        times.append(t_end-t_start)
        print(B2)
...
def testNUMBA():
    ...
    for i in range(10):
        t_start = time.perf_counter_ns()
        B3 = calculateFieldNumba(chopped, 1, (5+i, 5+i, 5+i))
        t_end = time.perf_counter_ns()
        times.append(t_end-t_start)
        print(B3[0])

    print("Times (ns): ", times)
    print("Average (post-init): {}".format(sum(times[1:])/len(times[1:])))
...
if __name__ == "__main__":
    cProfile.run("testSLICEONLY()")
    cProfile.run("testNEW()")
    cProfile.run("testOLD()")
    #cProfile.run("testNUMBA()")
